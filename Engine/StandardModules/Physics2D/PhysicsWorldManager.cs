...
public static class PhysicsWorldManager
{
    ...
    {
        ecsWorld.System("ClearActiveDimensionsSystem")
            .Kind(Ecs.PreUpdate)
            .Iter((Iter it) => { ActiveDimensions.Clear(); });

        // ARCHITECTURE FIX: Step 1 - Add dimensions containing ANY player
        ecsWorld.System<PhysicsDimension>("PopulatePlayerDimensionsSystem")
            .Kind(Ecs.PreUpdate)
            .With<CharacterClass>()
            .Each((ref PhysicsDimension dim) => { ActiveDimensions.Add(dim.Name); });

        // ARCHITECTURE FIX: Step 2 - Cascading Arbitration. If a Ship's interior is active, awake its exterior parent space!
        ecsWorld.System<PhysicsDimension, ShipVehicleComponent>("PopulateVehicleParentDimensionsSystem")
            .Kind(Ecs.PreUpdate)
            .Each((ref PhysicsDimension dim, ref ShipVehicleComponent ship) =>
            {
                if (ActiveDimensions.Contains(ship.InteriorDimensionName))
                {
                    ActiveDimensions.Add(dim.Name);
                }
            });
